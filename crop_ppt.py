...
def locate_ppt_and_save_debug_images(frame, debug_image_dir):
    """
    在单帧中定位PPT区域，并保存所有中间调试步骤的图像。
    
    参数:
        frame: 视频中的一帧图像 (numpy array)
        debug_image_dir: 保存调试图片的文件夹
    返回:
    ...
    # 0. 保存原始帧
    cv2.imwrite(os.path.join(debug_image_dir, "0_original_frame.jpg"), frame)
    print(f"  -> 已保存 '0_original_frame.jpg' 到 {debug_image_dir}")

    # 1. 预处理：灰度图
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    cv2.imwrite(os.path.join(debug_image_dir, "1_grayscale.jpg"), gray)
    ...
    cv2.imwrite(os.path.join(debug_image_dir, "2_canny_edges.jpg"), edged)
    ...
    cv2.imwrite(os.path.join(debug_image_dir, "3_all_contours_found.jpg"), contours_debug_image)
    ...
    for c in contours:
        ...
        if len(approx) == 4 and cv2.contourArea(c) > frame.shape[0] * frame.shape[1] * 0.1:
            ...
            cv2.imwrite(os.path.join(debug_image_dir, "4_final_result.jpg"), final_result_image)
...
def process_video(input_video_path):
    """处理单个视频文件：定位、裁剪并保存。"""
    print(f"\n{'='*20} 开始处理视频: {os.path.basename(input_video_path)} {'='*20}")
    
    # 1. 配置路径
    video_name = os.path.splitext(os.path.basename(input_video_path))[0]
    output_video_dir = os.path.join('output', 'video', video_name)
    output_video_path = os.path.join(output_video_dir, f"{video_name}_cropped.mp4")
    output_debug_image_dir = os.path.join('output', 'image', video_name)
    ...
    os.makedirs(output_debug_image_dir, exist_ok=True)
    ...
    if not bbox:
        print(f"\n错误：在 '{os.path.basename(input_video_path)}' 中多次尝试后仍自动定位失败。请检查调试图片。")
        cap.release()
        return
...
def main():
    ...
    for ext in video_extensions:
        input_video_paths.extend(glob.glob(os.path.join(input_dir, ext)))
